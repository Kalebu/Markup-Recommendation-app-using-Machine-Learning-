...
class Focus():
	...
	def __init__(self, root):
		self.sensor = pir.PIR
		self.lcd_display = LCD.show_results
		self.ocula = cv2
	...
	def develop_report(self, category):
		...
		if category:
			...
			self.lcd_display(class_name)#___showing result to lcd____________
			print(result)
	...
	def check_take_action(self, take=False, cancel=False, from_s = False):
		if take or self.sensor():#____________checking button or sensor input______
